...
public final class TomsStorageServerCompat {
    ...
    public static long syncSession(
        ...
        if (TomsStorageCompat.isTerminal(menuType)) {
            ...
            if (snapshot.hash() != previousHash) {
                byte[] data = TomsStoragePayloads.writeTerminalSnapshot(player.registryAccess(), snapshot.entries());
                ...
                if (TomsStoragePayloads.readTerminalSnapshot(player.registryAccess(), data).truncated()) {
                    DesktopDebug.warn(
        ...
        if (TomsStorageCompat.isMenu(menuType, TomsStorageCompat.INVENTORY_LINK)) {
            ...
            byte[] data = TomsStoragePayloads.writeLinkSnapshot(player.registryAccess(), channels, TomsStorageReflect.inventoryLinkSelected(menu));
    ...
    private static void handleTerminalAction(DesktopServerPayloadContext<AbstractContainerMenu> context) {
        ...
        TomsStoragePayloads.TerminalAction action = TomsStoragePayloads.readTerminalAction(context.player().registryAccess(), context.data());
        DesktopDebug.log(
    ...
    private static TerminalSnapshotData terminalSnapshot(AbstractContainerMenu menu) {
        ...
        for (TerminalEntry entry : entries) {
            hash = (hash ^ ItemStack.hashItemAndComponents(entry.stack())) * 1099511628211L;
