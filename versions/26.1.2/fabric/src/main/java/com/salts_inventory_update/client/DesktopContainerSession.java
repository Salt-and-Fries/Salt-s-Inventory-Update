        ...
        if (payload.specialKind() == DesktopPackets.SPECIAL_HORSE) {
            ...
            if (entity instanceof AbstractHorse horse) {
                return new HorseInventoryMenu(
                    payload.sessionId(),
                    player.getInventory(),
                    new SimpleContainer(AbstractMountInventoryMenu.getInventorySize(payload.columns())),
                    horse,
                    payload.columns()
                );
            }
        } else if (payload.specialKind() == DesktopPackets.SPECIAL_NAUTILUS) {
            Entity entity = minecraft.level == null ? null : minecraft.level.getEntity(payload.entityId());
            if (entity instanceof AbstractNautilus nautilus) {
                return new NautilusInventoryMenu(
                    payload.sessionId(),
                    player.getInventory(),
                    new SimpleContainer(AbstractMountInventoryMenu.getInventorySize(payload.columns())),
                    nautilus,
